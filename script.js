...
function renderNews(news) {
  ...
  if (sliderWrap) {
    ...
    sliderWrap.innerHTML = slideNews.map(n => {
      ...
      return `
      <div class="swiper-slide" onclick="window.open('${n.url}','_blank')" style="position:relative; overflow:hidden; background:#0d1320;">
        
        <div style="
          position: absolute; 
          inset: 0;
          background-image: url('${img}');
          background-size: auto 100%; /* Yüksekliği tam doldur, genişliği orantılı bırak */
          background-repeat: repeat-x; /* Görseli yatay eksende (x) yan yana tekrarla */
        ...
        <div class="slide-overlay" style="position:absolute; bottom:0; left:0; right:0; z-index:10; padding: 40px 22px 20px;">
          <div class="slide-tag">${n.source || 'Ekonomi'}</div>
          <div class="slide-title" style="color: white; text-shadow: 0px 2px 6px rgba(0,0,0,0.8);">${n.name}</div>
        </div>
      </div>`;
    }).join('');
